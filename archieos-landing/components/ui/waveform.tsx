...
export type LiveWaveformProps = Omit<WaveformProps, "data" | "onBarClick"> & {
  active?: boolean
  mode?: "static" | "scrolling"
  barCount?: number
  speed?: number
  prefill?: boolean // Start with bars already filling the width (for scrolling mode)
  minHeight?: number // Minimum bar height (0-1), for "flatline" effect
}
...
export const LiveWaveform = ({
  ...
  const seededRandom = useCallback((i: number, offset = 0) => {
    const x = Math.sin(seedRef.current * 10000 + i * 137.5 + offset) * 10000
    return x - Math.floor(x)
  }, [])

  // Initialize canvas and bars
  useEffect(() => {
    const canvas = canvasRef.current
    const container = containerRef.current
    if (!canvas || !container) return
    ...
    const resizeObserver = new ResizeObserver(() => {
      ...
      // Initialize bars - prefill entire width if prefill is true
      if (barsRef.current.length === 0) {
        const step = barWidth + barGap
        if (prefill) {
          // Fill from left edge (0) to right edge (rect.width)
          let currentX = 0
          let index = 0
          while (currentX < rect.width + step) {
            barsRef.current.push({
              x: currentX,
              height: minHeight + seededRandom(index++) * (0.6 - minHeight),
            })
            currentX += step
          }
        } else {
          // Start from right edge (original behavior)
          let currentX = rect.width
          let index = 0
          while (currentX > -step) {
            barsRef.current.push({
    ...
    resizeObserver.observe(container)
    return () => resizeObserver.disconnect()
  }, [barWidth, barGap, prefill, minHeight, seededRandom])
  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      ...
      if (mode === "scrolling" && active) {
        // Move bars from right to left
        for (let i = 0; i < barsRef.current.length; i++) {
          barsRef.current[i].x -= speed * deltaTime
        }

        // Remove bars that have scrolled off the left edge
        barsRef.current = barsRef.current.filter(
          (bar) => bar.x + barWidth > -step
        )

        // Add new bars on the right edge as needed
        while (
          barsRef.current.length === 0 ||
          barsRef.current[barsRef.current.length - 1].x < rect.width
        ) {
          ...
          // Generate organic wave-like heights
          const time = currentTime * 0.001
          const uniqueIndex = barsRef.current.length + time * 0.01
          const wave1 = Math.sin(uniqueIndex * 0.1) * 0.2
          const wave2 = Math.cos(uniqueIndex * 0.05) * 0.15
          const randomComponent = seededRandom(uniqueIndex, time) * 0.4
      ...
      } else if (mode === "static" && active) {
        // Static mode: bars pulse in place
        for (let i = 0; i < barsRef.current.length; i++) {
          const wave1 = Math.sin(currentTime * 0.002 + i * 0.2) * 0.15
          const wave2 = Math.cos(currentTime * 0.001 + i * 0.1) * 0.1
          const random = seededRandom(i, currentTime * 0.001) * 0.3
          const targetHeight = Math.max(minHeight, Math.min(0.9, 0.35 + wave1 + wave2 + random))
          // Smooth interpolation
          barsRef.current[i].height += (targetHeight - barsRef.current[i].height) * 0.1
        }
      }
